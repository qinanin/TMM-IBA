...
from ViT_explanation_generator import Baselines, LRP,IBA
...
def compute_saliency_and_save(args):
    ...
    with h5py.File(os.path.join(args.method_dir, 'results.hdf5'), 'a') as f:
        ...
        for batch_idx, (data, target) in enumerate(tqdm(sample_loader)):
            if first:
                first = False
                data_cam.resize(data_cam.shape[0] + data.shape[0] - 1, axis=0) #(1, 1, 224, 224)
                data_image.resize(data_image.shape[0] + data.shape[0] - 1, axis=0) #(1, 3, 224, 224)
            ...
            # Add data
            data_image[-data.shape[0]:] = data.data.cpu().numpy()  #是赋值给最后一行的意思
            ...
            if args.method == 'rollout':
                ...
                # Res = Res - Res.mean()
            elif args.method == 'IBA':
                Res = iba.generate_LRP(data, start_layer=1, method="IBA", index=index).reshape(data.shape[0], 1, 14, 14)
            ...
            data_cam[-data.shape[0]:] = Res.data.cpu().numpy() # Res.shape torch.Size([1, 1, 224, 224])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method', type=str,
                        default='lrp_last_layer',
                        choices=['rollout', 'lrp', 'transformer_attribution', 'full_lrp', 'lrp_last_layer',
                                 'attn_last_layer', 'attn_gradcam','IBA'],
                        help='')
    ...
    parser.add_argument('--is-ablation', type=bool,
                        default=False,
                        help='')
    parser.add_argument('--imagenet-validation-path', type=str,default='',help='')
