...
parser.add_argument('--method', type=str,
                    default='IBA',
                    choices=[ 'rollout', 'lrp','transformer_attribution', 'full_lrp', 'lrp_last_layer','block_full_lrp',
                              'attn_last_layer', 'attn_gradcam', 'attn_gradcam_me','IBA'],
                    help='')
...
def eval_batch(image, labels, evaluator, index):
    ...
    if args.method == 'rollout':
        ...
        Res = lrp.generate_LRP(image.cuda(), start_layer=1, index=index).reshape(batch_size, 1, 14, 14)
    ...
    # segmentation test for our method
        ...
        Res = iba.generate_LRP(image.cuda(), start_layer=1, method="IBA").reshape(batch_size, 1, 14, 14)
    ...
    elif args.method == 'attn_gradcam_me':
        Res = baselines.generate_cam_attn1(image.cuda()).reshape(batch_size, 1, 14, 14)

    if args.method != 'full_lrp' and args.method!='block_full_lrp':
