...
class Attention(nn.Module):
    ...
    def forward(self, x,one_batch_attn=None):
        ...
        return out #torch.Size([16, 197, 768])
...
class Block(nn.Module):
    ...
    def forward(self, x,one_batch_attn=None):
        ...
        x = self.add1([x1, self.attn(self.norm1(x2),one_batch_attn)])
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x,tar_layer=None,one_batch_attn=None,index=None):
        B = x.shape[0]
    ...
    def relprop(self, cam=None,method="transformer_attribution", is_ablation=False, start_layer=0,choose_layer=0, **kwargs):
        cam = self.head.relprop(cam, **kwargs)
        cam = cam.unsqueeze(1)
        cam = self.pool.relprop(cam, **kwargs)
        cam = self.norm.relprop(cam, **kwargs)
        if method=="IBA":
            blk=self.blocks[choose_layer]
