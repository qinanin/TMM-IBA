...
class LRP:
    ...
    def generate_LRP(self, input, index=None, method="transformer_attribution", is_ablation=False, start_layer=0):
        ...
        one_hot = torch.sum(one_hot.cuda() * output) #1 1000
...
class IBA:
    ...
    def generate_LRP(self, input, index=None, method="IBA", is_ablation=False, start_layer=0):
        output = self.model(input)
        kwargs = {"alpha": 1}
        if index == None:
            index = np.argmax(output.cpu().data.numpy(), axis=-1)
        ...
        for blk in blocks:
            ...
            one_hot_vector = one_hot
            one_hot = torch.from_numpy(one_hot).requires_grad_(True)
            one_hot = torch.sum(one_hot.cuda() * prob[k])
            self.model.zero_grad() 
            one_hot.backward(retain_graph=True) 
            self.model.relprop(torch.tensor(one_hot_vector).to(input.device),method=method,is_ablation=is_ablation,start_layer=start_layer,choose_layer=k,**kwargs)
            attn=blk.attn.get_attn()
            cam=blk.attn.get_attn_cam() 
            cam=cam[0]
            gradient=torch.autograd.grad(torch.unbind(prob[k][:,index]),attn,retain_graph=True)[0][0] 
            cam1=gradient*cam
            cam1=cam1.clamp(min=0).mean(dim=0)
            cam_.append(cam1.unsqueeze(0))
            k=k+1
        rollout=compute_rollout_attention(cam_,start_layer=start_layer)
        cam1=rollout[:,0,1:]
        return cam1
    

def scaled_input(emb, baseline=None,start_i=None, end_i=None):
    if baseline is None:
